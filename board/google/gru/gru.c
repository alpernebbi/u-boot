...
int rk_board_late_init(void)
{
	...
	if (ret) {
		...
		printf("Trying to set cros_ec_pwm channel 0 duty\n");
		ret = pwm_set_config(cros_ec_pwm, 0, 0xffff, 0x4fff);
		if (ret) {
			printf("Failed to set cros_ec_pwm channel 0 duty: %d\n", ret);
			return 0;
		}

		printf("Trying to enable cros_ec_pwm channel 0\n");
		ret = pwm_set_enable(cros_ec_pwm, 0, true);
		if (ret) {
			printf("Failed to enable cros_ec_pwm channel 0: %d\n", ret);
			return 0;
		}
	}
