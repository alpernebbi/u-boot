...
'  fdtmap             90   243  fdtmap          90',
    ...
    def testDepthchargeKernel(self):
        ...
        data = self._DoReadFileDtb(
            '221_depthcharge_kernel.dts', entry_args=entry_args)[0]
        expected = (
            VBLOCK_DATA +
            tools.GetBytes(0, 0x10000 - len(VBLOCK_DATA)) +
            U_BOOT_DATA +
            tools.GetBytes(0, 0x1000 - len(U_BOOT_DATA)) +
            tools.ToBytes(TEXT_DATA) +
            tools.GetBytes(0, 0x2000 - len(TEXT_DATA)) +
            U_BOOT_SPL_DATA +
            tools.GetBytes(0, 0x1000 - len(U_BOOT_SPL_DATA))
        )
        self.assertEqual(expected, data)
