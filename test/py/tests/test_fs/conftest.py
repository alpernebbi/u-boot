...
def fs_obj_basic(request, u_boot_config):
    ...
    try:
        ...
        # 3GiB volume
        fs_img = util.mk_fs(u_boot_config, fs_type, 0xc0000000, '3GB')
    ...
    mount_context = util.mount_fs(fs_type, fs_img, mount_dir)
    try:
...
def fs_obj_ext(request, u_boot_config):
    ...
    try:
        ...
        # 128MiB volume
        fs_img = util.mk_fs(u_boot_config, fs_type, 0x8000000, '128MB')
    except CalledProcessError as err:
        pytest.skip('Creating failed for filesystem: ' + fs_type + '. {}'.format(err))
        return
...
def fs_obj_symlink(request, u_boot_config):
    ...
    try:
        ...
        # 1GiB volume
        fs_img = util.mk_fs(u_boot_config, fs_type, 0x40000000, '1GB')
    except CalledProcessError as err:
        pytest.skip('Creating failed for filesystem: ' + fs_type + '. {}'.format(err))
        return
